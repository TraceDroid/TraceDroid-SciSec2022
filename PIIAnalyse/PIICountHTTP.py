...
def get_http_message(db_connection):
    select_sql = "select requestHeaders, requestBody, URL from HTTP where protocol='HTTP'"
    # print(select_sql)
...
if __name__ == '__main__':
    ...
    with open("PIIAnalyse2.csv", "w", newline="", encoding="utf-8-sig") as f:
        csv_writer = csv.writer(f)
        csv_writer.writerow(["HTTP", "device info", "location info", "network info"])
        for position in pii_count.keys():
            result = [position]
